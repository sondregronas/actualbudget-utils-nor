...
) -> list[tuple[Transactions, Transactions]]:
        ...
        for other in transactions:
            ...
            if any(
                [
                    ...
                    not other.amount,
                    transaction.acct == other.acct,
                    not transaction.payee,
                    not other.payee,
                ]
            ):
                continue
            ...
            if all(
                [
                    transaction.date == other.date,
                    transaction.amount == -other.amount,
                    not transaction.payee.transfer_acct,
                    not other.payee.transfer_acct,
                ]
            ):
